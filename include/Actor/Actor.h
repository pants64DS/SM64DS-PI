...
struct Actor : ActorDerived // internal name: dActor
{
	enum Flags : s32
	{
		...
		UPDATE_DURING_DIALOGUE              = 1 << 23,
		CAN_NO_DAMAGE_SQUISH_PLAYER         = 1 << 24,
		CAN_SQUISH                          = 1 << 25,
		UPDATE_DURING_STAR_SPAWNING         = 1 << 26,
		SPAWN_EVEN_IF_KILLED_BEFORE         = 1 << 27,
		AIMABLE_BY_EGG                      = 1 << 28,
		UPDATE_DURING_CUTSCENES             = 1 << 29,
	...
	virtual void OnGroundPounded(Actor& attacker);
	virtual void OnHitFromSide(Actor& attacker);
	virtual void OnPunched(Actor& attacker);
	virtual void OnKicked(Actor& attacker);
	virtual void OnPushed(Actor& attacker);
	virtual void OnHitByCannonBlastedChar(Actor& attacker);
	virtual void OnHitByMegaChar(Actor& attacker);
	...
	void PlayHitSound(u32 hitFlags); //NCS_SE_PT_HIT_S if HIT_BY_PUNCH, NCS_SE_PT_HIT_H if HIT_BY_KICK, HIT_BY_SWEEP_KICK or HIT_BY_SLIDE_KICK
	...
	void GivePlayerCoins(Player& player, u32 numCoins, u32 coinType);

	// C = closest, F = farthest
	Fix12i DistToCPlayer() const;
	Player* ClosestPlayer() const;
	Player* ClosestNonVanishPlayer() const;
	s16 HorzAngleToCPlayer() const;
	s16 HorzAngleToCPlayerOrAng() const;
	Player* FarthestPlayer() const;
	s16 HorzAngleToFPlayer() const;
	bool IsPlayerInRange(s32 maxDist) const; // yes it uses an int
	bool IsPlayerInRange(const Vector3& pos, s32 maxDist) const; // uses pos instead of this->pos
	...
	static Actor* Spawn(u32 actorID, u32 param1, const Vector3& pos, const Vector3_16* rot = nullptr, s32 areaID = 0, s32 deathTableID = -1);
	...
	Actor* FindClosestWithActorID(u32 actorID);
	static Actor* FindWithID(u32 id);
	static Actor* FindWithActorID(u32 actorID, Actor* searchStart = nullptr); //searchStart is not included.
	...
	[[gnu::always_inline]]
	void UpdateVertSpeedWithNormal(const Vector3& normal)
	{
		if (normal.y != 0._f)
		{
	...
	template<class T>
	static auto* Spawn(u32 param1, const Vector3& pos, auto... args)
	{
